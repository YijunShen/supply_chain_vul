...
class Jardealer:
    ...
    def __init__(self, txtdirname="cgtxt/", jsondirname="classjson/", log=True):
        self.jarroot = dataPath + "/jars/"
        self.cgroot = dataPath + "/cgdata/"
    ...
    def deal_jars_under(self, cgdealer, path):
        ...
        filename = self.jarroot + "fail_cg.json"
    ...
    def del_dep_jars(self, cgdealer, group, artifact, version):
        ...
        ed = time.time()
        rlt = ("deal deps %s:%s:%s, %s deps, %s succ, %s dup, %s fail, %s miss in %s s" %
               (group, artifact, version, depcnt, depsuccnt, depdupcnt, depfailcnt, depmisscnt, ed - st))
        if self.log:
    ...
    def deal_pkg_dep_jars(self, cgdealer, group, artifact, version, dealdepdep=True):
        ...
        if dealdepdep:
            ...
            info2 = ("----DEALED DEPDEP %s:%s:%s, %d root dep -> %d dep dep" %
                     (group, artifact, version, rootdep, totaldep - rootdep))
            if self.log:
                self.pkgDepLogger.critical(info2)
            print(info2)
        
        ed = time.time()
        rlt = ("deal pkgdeps %s:%s:%s, deal dep dep: %s, %d root dep -> %d dep dep, in %s s" %
               (group, artifact, version, dealdepdep, rootdep, totaldep - rootdep, ed - st))
        if self.log:
            self.pkgDepLogger.critical(rlt)
        return True, rlt
        
    def getClassPkgDict(self, group, artifact, version, cgdealer=srcPath+"/util/java_cg.jar"):
        rlt = {}
