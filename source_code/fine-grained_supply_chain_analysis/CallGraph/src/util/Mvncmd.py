...
srcPath = os.path.split(curPath)[0].replace("\\", "/")
dataPath = os.path.split(srcPath)[0] + "/data"
...
class Mvncmd:
        ...
        try:
            ...
            for i in range(3):
                output = subprocess.check_output(
                    "cd {} && mvn dependency:list -DincludeScope={} -DexcludeTransitive={}".
                    format(pomPath, scope, excludeTransitive), shell=True, stderr=subprocess.DEVNULL)
    ...
    def getDepJarDirect(self, group, artifact, version, rootpath=dataPath+"/jars/"):
    ...
    def getJarDefault(self, group, artifact, version, rootpath=dataPath+"/jars/"):
        grouppath = group.replace(".", "/")
        ...
        try:
            ...
            for i in range(3):
                output = subprocess.check_output(
                    "mvn dependency:get -Dartifact={}:{}:{}:jar -Dtransitive=false -Ddest={}".
                    format(group, artifact, version, filename), shell=True, stderr=subprocess.DEVNULL)
                output = output.decode('utf-8')
                if "FAILURE" in output:
                    continue
                else:
    ...
    def getPomDefault(self, group, artifact, version, rootpath=dataPath+"/jars/"):
        ...
        try:
            ...
            for i in range(3):
                output = subprocess.check_output(
                    "mvn dependency:get -Dartifact={}:{}:{}:pom -Dtransitive=false -Ddest={}".
                    format(group, artifact, version, filename), shell=True, stderr=subprocess.DEVNULL)
                output = output.decode('utf-8')
    ...
    def downloadInfo(self, group, artifact, version, downloadDeps=True, downloaddepdeps=True):
        ...
        rlt = ("download %s jar and pom, t1: %f s, deps: %s, t2: %f s, downlaod depdep: %s,"
               " t3: %f s, root dep cnt: %d, total dep cnt: %d, time: %f s" %
               (pkg, ed1 - st, deprlt, ed3 - ed2, downloaddepdeps, ed4 - ed3, rootdep, totaldep, ed - st))

        if self.logger:
            self.logger.critical(rlt)
        return rlt
    
    def getPkgRoot(self, group, artifact, version, rootpath=dataPath+"/jars/"):
        grouppath = group.replace(".", "/")
        pkgroot = rootpath + grouppath + "/" + artifact + "/" + version + "/" 
        return pkgroot
    
    def getPkgJarPath(self, group, artifact, version):
        root = self.getPkgRoot(group, artifact, version)
        jarName = group + "--" + artifact + '--' + version + ".jar"
        jarPath = root + jarName
        return jarPath
    
    def getPkgPomPath(self, group, artifact, version):
        root = self.getPkgRoot(group, artifact, version)
        pomPath = root + "pom.xml"
        return pomPath 
    
    def getPkgDepJsonPath(self, group, artifact, version):
        root = self.getPkgRoot(group, artifact, version)
        DepjsonPath = root + "deps.json"
        return DepjsonPath
    
    def getPkgDirectDepJsonPath(self, group, artifact, version):
        root = self.getPkgRoot(group, artifact, version)
        directDepjsonPath = root + "directDeps.json"
        return directDepjsonPath

    def getPkgTransDepJsonPath(self, group, artifact, version):
        root = self.getPkgRoot(group, artifact, version)
