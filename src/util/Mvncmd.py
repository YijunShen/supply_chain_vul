...
class Mvncmd:
    ...
    def getDepJarDirect(self, group, artifact, version, rootpath = "/data/jars/", scope = "compile", excludeTransitive = "false"):
    ...
    def getJarDefault(self, group, artifact, version, rootpath = "/data/jars/"):
    ...
    def getPomDefault(self, group, artifact, version, rootpath = "/data/jars/"):
        grouppath = group.replace(".", "/")
        newpath = rootpath + grouppath + "/" + artifact + "/" + version + "/"
        if not os.path.exists(newpath):
            os.makedirs(newpath) 
    ...
    def getPkgRoot(self, group, artifact, version, rootpath = "/data/jars/"):
        grouppath = group.replace(".", "/")
        pkgroot = rootpath + grouppath + "/" + artifact + "/" + version + "/" 
        return pkgroot
    
    def getPkgJarPath(self, group, artifact, version, rootpath = "/data/jars/"):
        root = self.getPkgRoot(group, artifact, version)
        jarName = group + "--" + artifact + '--' + version +".jar"
        jarPath = root + jarName
        return jarPath
    
    def getPkgPomPath(self, group, artifact, version, rootpath = "/data/jars/"):
        root = self.getPkgRoot(group, artifact, version)
        pomPath = root + "pom.xml"
        return pomPath 
    
    def getPkgDepJsonPath(self, group, artifact, version, rootpath = "/data/jars/"):
        root = self.getPkgRoot(group, artifact, version)
        DepjsonPath = root + "deps.json"
        return DepjsonPath
    
    def getPkgDirectDepJsonPath(self, group, artifact, version, rootpath = "/data/jars/"):
        root = self.getPkgRoot(group, artifact, version)
        directDepjsonPath = root + "directDeps.json"
        return directDepjsonPath

    def getPkgTransDepJsonPath(self, group, artifact, version, rootpath = "/data/jars/"):
        root = self.getPkgRoot(group, artifact, version)
